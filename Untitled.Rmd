...
Por un lado, puedes construir gráficos usando solamente `plotly` y mediante una sintaxis muy similar a la de `ggplot2`. De manera general se usa siempre la función `plot_ly()` y dentro el data.frame en `data`, los ejes x y/o y, color... y el tipo de gráfico se especifica a través de los parámetros `type` y `mode`.
...
```{r ejemplo_22}
api_create(plot_ly(data=df, x=~attack, y=~sp_attack, split=~is_legendary, type='scatter', mode='markers'), filename='pokemons')
```
